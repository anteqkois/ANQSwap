...
contract('ANQSwap', (accounts) => {
  ...
  describe('Buy token', async () => {
    it('User can buy ANQ', async () => {
      const predirectAmountOfBuyTokens = await anqSwap.predirectExactAmountOut(web3.utils.toWei(web3.utils.toBN(5), 'ether'), true);
      ...
      assert.equal(web3.utils.fromWei(logs[0].args._amountANQ), web3.utils.fromWei(predirectAmountOfBuyTokens));
      assert.equal(web3.utils.fromWei(logs[0].args._amountETH), 5);
  ...
  describe('Sell token', async () => {
    it('User can sell ANQ', async () => {
      ...
      const predirectAmountOfETHBack = await anqSwap.predirectExactAmountOut(balanceOfANQSeller, false);
      ...
      assert.equal(web3.utils.fromWei(logs[0].args._amountETH, 'ether'), fromWei(predirectAmountOfETHBack));
      assert.equal(web3.utils.fromWei(balanceOfUserANQ, 'ether'), 0);
      assert.equal(web3.utils.fromWei(balanceOfSwapANQ, 'ether'), 100_000);
      assert.equal(web3.utils.fromWei(balanceOfSwapEther, 'ether'), 55 - web3.utils.fromWei(predirectAmountOfETHBack, 'ether'));
    });
